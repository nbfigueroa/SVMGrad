 ...
 * 
 *  mx_calculate_classifier_gradient.cpp
 *
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
	...
	{
		printf("Usage: mx_calculate_classifier_gradient(svm_struct, data_set)\n");
		return;
	}
	...
	plhs[0] = mxCreateDoubleMatrix(dim, npoints, mxREAL);
	double* derptr = mxGetPr(plhs[0]);

	for(int i=0;i<npoints;i++)
		svm->getclassifierderivative(datasetarray + dim*i, derptr +dim*i);
}
